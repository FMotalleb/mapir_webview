...
Page resource error:
  ...
  isForMainFrame: ${error.isForMainFrame}''',
    ...
    on<IInjectableJsEvent>(
      (event, emit) async {
        final logger = _createLoggerFor(event);
        while (isMapInitialized == false && event.shouldRunAfterMapInitialized) {
        ...
        _applyRotation: function () {
            if(this.options.rotationAngle) {
                ...
                if(oldIE) {
                    ...
                    this._icon.style[L.DomUtil.TRANSFORM] = 'rotate(' + this.options.rotationAngle + 'rad)';
                } else {
                    // for modern browsers, prefer the 3D accelerated version
                    this._icon.style[L.DomUtil.TRANSFORM] += ' rotateZ(' + this.options.rotationAngle + 'rad)';
                }
            }
        },
